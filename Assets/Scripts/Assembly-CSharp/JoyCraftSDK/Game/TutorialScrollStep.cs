...
{
	public class TutorialScrollStep : JMonoBehaviour, IStep, IStepEvent
	{
		...
		[Tooltip("Tay CAO HƠN mép ĐÁY tranh bao nhiêu, theo tỉ lệ chiều cao tranh (nhắm đúng hàng đáy cần cào thủng).")]
		[Range(0f, 0.3f)]
		[SerializeField]
		private float handAboveBottomFraction;

		[Tooltip("Quãng tay đi NGANG so với bề ngang tranh (1 = trọn mép trái -> mép phải).")]
		[Range(0.1f, 1f)]
		[SerializeField]
		...
		private void GetPictureColumnScrolled(ref AEventManager.OnPictureColumnScrolled eventData)
		{
		}
